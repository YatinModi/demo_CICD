...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    var FirebaseAllDataArr = [firebaseDataModel]()
    var isSelectedCategory = ""
    var isSelectedArea = ""
    var isSelectedSection = ""
    var ArrSelectedAmenities : NSMutableArray = []
    var PercentageValue : CGFloat = 0.0
    var strCity = "Mommydata"

    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        guard let _ = (scene as? UIWindowScene) else { return }
    }

    func navigationControll(identifier: String,storyboardName: String,msg:String){
        ...
        if msg != ""{
            initialViewController.view.makeToast(msg)
        }
